...
(s/fdef pprint-dispatch :args (s/cat :x any?) :ret nil?)
(s/fdef
  write-object
  :args
  (s/cat
    :m
    (s/map-of (s/or :string? string? :nil? nil? :int? int?) int?)
    :out
    (partial instance? java.io.PrintWriter))
  :ret
  nil?)
(s/fdef
  pprint-generic
  ...
  (s/cat
    :x
    (s/or :string? string? :boolean? boolean? :number? number?))
  :ret
  nil?)
(s/def ::alias__25389 (s/or :G__32842 (s/coll-of int?) :int? int?))
(s/def ::alias__25391 (s/or :char? char? :G__32843 (s/coll-of any?)))
(s/fdef
  codepoint-clause
  :args
  (s/cat :codepoint-clause-0 (s/coll-of any?))
  :ret
  (s/tuple ::alias__25389 ::alias__25391))
(s/fdef
  write-null
  :args
  (s/cat :x nil? :out (partial instance? java.io.PrintWriter))
  :ret
  nil?)
(s/fdef
  write-plain
...
(s/fdef
  write-bignum
  :args
  (s/cat :x number? :out (partial instance? java.io.PrintWriter))
  :ret
  nil?)
(s/fdef pprint-array :args (s/cat :s (s/coll-of any?)) :ret nil?)
(s/fdef
  read-hex-char
  :args
  (s/cat :stream (partial instance? java.io.PushbackReader))
  :ret
  char?)
(s/fdef
  write-double
...
(s/fdef
  ...
  (s/alt
    :3-args
    (s/cat :x string? :& keyword? :options false?)
    :1-args
    (s/cat :pprint-0 (s/coll-of any?)))
  :ret
  nil?)
(s/def ::alias__25494 string?)
(s/def ::alias__25497 keyword?)
(s/def
  ::alias__25453
  (s/or
    :G__32859
    (partial instance? java.sql.Date)
    :G__32860
    (partial instance? java.lang.Object)
    :int?
    int?))
(s/def ::alias__25496 any?)
(s/def ::alias__25462 any?)
(s/def ::alias__25442 keyword?)
(s/def ::alias__25461 any?)
(s/def ::alias__25458 keyword?)
(s/def
  ::alias__25460
  (s/or
    :G__32861
    ...
    :int?
    int?))
(s/def ::alias__25459 (s/or :string? string? :int? int?))
(s/def ::alias__25451 keyword?)
(s/def ::alias__25441 string?)
(s/def ::alias__25463 (s/and (partial instance? java.lang.Object)))
(s/def ::alias__25495 keyword?)
(s/def ::alias__25452 (s/or :string? string? :int? int?))
(s/fdef
  read
  :args
  (s/alt
    ...
    (s/cat
      ...
      keyword?
      :read-2
      (s/or :ifn? ifn? :false? false?)
      :read-3
      keyword?
      :read-4
      (s/or :keyword? keyword? :ifn? ifn?))
    :3-args
    (s/cat
      :reader
      (partial instance? java.io.StringReader)
    ...
    (s/cat
      ...
      (s/or
        ...
        (partial instance? java.io.StringReader)
        :G__32858
        (partial instance? java.io.PushbackReader))))
  :ret
  any?)
(s/fdef
  write-ratio
...
(s/fdef
  ...
  :ret
  (s/map-of string? any?))
(s/def ::alias__26347 int?)
(s/def ::alias__26344 (s/or :nil? nil? :number? number?))
(s/def
  ::alias__26306
  (s/or
    :G__32875
    ...
    :int?
    int?))
(s/def ::alias__26327 (s/or :nil? nil? :number? number?))
(s/def ::alias__26348 any?)
(s/def ::alias__26346 any?)
(s/def ::alias__26305 (s/or :nil? nil? :number? number?))
(s/def
  ::alias__26345
  (s/or
    :G__32876
    ...
    :int?
    int?))
(s/def ::alias__26343 keyword?)
(s/def ::alias__26326 keyword?)
(s/def ::alias__26304 keyword?)
(s/def
  ::alias__26328
  (s/or
    :G__32877
...
(s/fdef
  ...
  (s/cat
    ...
    (s/or
      ...
      set?
      :G__32878
      (s/coll-of (s/or :nil? nil? :int? int?)))
    :out
    (partial instance? java.io.PrintWriter))
  :ret
  nil?)
...
(s/fdef
  ...
  (s/cat
    :x
    (partial instance? (Class/forName "[Ljava.lang.Long;"))
    :out
    (partial instance? java.io.PrintWriter))
  :ret
  nil?)
(s/def ::alias__27650 (s/or :nil? nil? :number? number?))
(s/def ::alias__27655 keyword?)
(s/def
  ::alias__27651
  (s/or
    :G__32888
    ...
    :int?
    int?))
(s/def ::alias__27660 any?)
(s/def ::alias__27656 number?)
(s/def ::alias__27657 (s/and (partial instance? java.lang.Object)))
(s/def ::alias__27649 keyword?)
(s/def ::alias__27659 keyword?)
(s/def ::alias__27661 any?)
(s/fdef
  write
  :args
  (s/alt
...
(s/fdef
  ...
  :ret
  string?)
(s/def ::alias__27977 keyword?)
(s/def ::alias__28009 keyword?)
(s/def ::alias__28008 string?)
(s/def ::alias__27976 any?)
(s/def ::alias__28013 keyword?)
(s/def ::alias__27993 keyword?)
(s/def
  ::alias__27995
  (s/or
    :G__32899
    ...
    :int?
    int?))
(s/def ::alias__28010 string?)
(s/def ::alias__27994 (s/or :string? string? :int? int?))
(s/def ::alias__27975 keyword?)
(s/def ::alias__27974 string?)
(s/def ::alias__28011 keyword?)
(s/def ::alias__28012 any?)
(s/fdef
  read-str
  :args
  (s/alt
    :1-args
    (s/cat :read-str-0 string?)
    :5-args
    (s/cat
      :read-str-0
      string?
...
(s/fdef
  ...
  (s/cat
    :k
    (s/or :string? string? :keyword? keyword? :int? int?)
    :v
    any?)
  :ret
  any?)
...
(s/fdef
  write-string
  :args
  (s/cat :s string? :out (partial instance? java.io.PrintWriter))
  :ret
  nil?)
(s/fdef read-decimal :args (s/cat :string string?) :ret number?)
(s/fdef
  write-named
  :args
  (s/cat :x symbol? :out (partial instance? java.io.PrintWriter))
  :ret
  nil?)
(s/fdef
  read-integer
  ...
  (s/cat :string string?)
  :ret
  (s/or :number? number? :int? int?))
(s/fdef
  default-write-key-fn
  :args
  (s/cat
    :x
    (s/or :string? string? :keyword? keyword? :nil? nil? :int? int?))
  :ret
  string?)
